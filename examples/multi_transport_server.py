...
@click.option(
    ...
    type=int,
    default=8000,
    help="Port for StreamableHTTP transport"
)
@click.option(
    "--webhook-port",
    type=int,
    default=8001,
    help="Port for Webhook transport"
)
@click.option(
    "--stateless",
    is_flag=True,
...
def main(http_port, webhook_port, stateless) -> int:
    ...
    async def handle_tools(name: str, arguments: dict) -> list[types.ContentBlock]:
        if name == "fetch_sync":
            ...
            return await fetch_sync(arguments["url"])
        elif name == "analyze_async":
            if "url" not in arguments:
                raise ValueError("Missing required argument 'url'")
            return await analyze_async(arguments["url"])
    ...
    async def arun():
        ...
        multi_server = MultiTransportServer(
            mcp_server=app,
