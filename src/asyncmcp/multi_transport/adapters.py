...
class WebhookTransportAdapter(TransportManager):
    ...
    def __init__(self, webhook_manager: Any, external_management: bool = False):
        self.webhook_manager = webhook_manager
        self.external_management = external_management
        self._is_running = False
        self._context_manager = None
    ...
    async def start(self) -> None:
        ...
        try:
            if self.external_management:
                # External management - don't start the webhook manager
                logger.info("Webhook transport managed externally")
                self._is_running = True
            else:
                # Start the webhook manager using async context manager
    ...
    async def stop(self) -> None:
        ...
        try:
            if self.external_management:
                # External management - don't stop the webhook manager
                logger.info("Webhook transport externally managed, skipping stop")
                self._is_running = False
            else:
                if self._context_manager:
                    try:
